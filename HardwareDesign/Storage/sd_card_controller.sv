...
module sd_controller(
    ...
    logic [6:0] clk_div_counter = 0;
    logic clk_400khz = 0;

    always @(posedge clk) begin
        if (clk_div_counter == 31) begin
            clk_div_counter <= 0;
    ...
    logic mul_clk;
    assign mul_clk = is_initialized ? clk_25mhz_en : clk_400khz;

    reg [7:0] init_retry_counter = 0;

    parameter INIT_RETRY_LIMIT = 50;
    ...
    parameter RECOVERY_START = 22;
    parameter RECOVERY_CLOCKS = 23;
    ...
    always @(posedge mul_clk) begin
        if (reset == 1) begin
            ...
            case (state)
                ...
                CMD55: begin
                    if (init_retry_counter >= INIT_RETRY_LIMIT) begin
                        state <= RECOVERY_START; 
                        is_initialized <= 0;
                    end else begin
                        init_retry_counter <= init_retry_counter + 1;
                ...
                RECOVERY_START: begin
                    cs <= 1;
                    bit_counter <= 1000;
                    sclk_sig <= 0;
                    state <= RECOVERY_CLOCKS;
                end

                RECOVERY_CLOCKS: begin
                    if (bit_counter == 0) begin
                        cs <= 0;
                        state <= RST;
                    end else begin
